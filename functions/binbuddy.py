...
class BinBuddy:
    ...
    def __init__(self, config=None):
        ...
        self.client = boto3.client('rekognition', aws_access_key_id=self.config['rekognition']['aws_access_key_id'],
                                   aws_secret_access_key=self.config['rekognition']['aws_secret_access_key'], region_name=self.config['rekognition']['region_name'])
    ...
    def which_bin_to_thrash_image(self, image_byte):
        ...
        for label in labels:
            ...
            if bin_type > 0:
                text = 'ตรวจพบ "{}" เป็นขยะประเภท "{}"'.format(
                    detected_object, bin_type_text)
